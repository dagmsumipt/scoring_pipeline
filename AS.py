...
def feature_selection(X_train, y_train, X_test, y_test, n_features = 10, selection_type = 'sw', try_all = False):
    ...
    if selection_type == 'sw':
        ...
        included = IDF.stepwise_selection(X_train, y_train, threshold_in=0.01, threshold_out = 0.05, verbose=False)
        print('done included')
...
def delete_correlated_features(df, cut_off=0.75, is_plot_prev=True, exclude = []):
...
def plot_bin(ev, for_excel=False):
    ...
    plt.savefig('{}.png'.format(ev.VAR_NAME[0]), dpi=100, bbox_inches = 'tight')
    plt.show()    
    
def plot_all_bins(iv_df, X_train):
...
def create_gini_stability(df, clf_lr, X_train):
    df3 = df.copy()
    months = sorted(list(df3.requested_month_year.drop_duplicates()))
    Ginis = pd.DataFrame()
    Ginis['Months'] = df3.requested_month_year.drop_duplicates().sort_values().reset_index(drop=True)
    Ginis['Gini'] = 1
    ...
    for i in range(len(months)):
        try:
            q.append(round(2*roc_auc_score(df3[df3.requested_month_year==months[i]].target, clf_lr.predict_proba(df3[df3.requested_month_year==months[i]][list(X_train.columns)])[:,1])-1, 3))
        except:
        ...
        try:
            ...
            goods.append('--')            
        try:
            bads.append(df3[df3.requested_month_year==months[i]].target.value_counts()[1])
        except:            
...
def gini_stability_chart(Ginis):
    ...
    plt.ylim([0, 1])
    plt.legend((p2[0], p1[0]), ('bad', 'good'), loc='best', fontsize=10)
    plt.savefig('gini_stability.png', dpi=100)
    plt.show()
    
def score_stability_graph(table):
...
def population_stability(df3, ivs, feat):    
    # Calculate PSI
    ...
    for j in features_of_model:
        ...
        q = list()
        for i in range(len(months)):
            q.append(calculate_psi(df3[df3.requested_month_year==months[i]][j], df3[df3.requested_month_year==months[0]][j]))
        psi[j] = q
        q = list()
    ...
    for col in features_of_model:
        ...
        plt.ylim([0, 0.3])
        plt.savefig('Stability_of_{}.png'.format(table.columns.names[0]), dpi=100, bbox_inches = 'tight')
        plt.show()
...
def export_to_excel(DSL, SQL, X_train, X_test, y_train, y_test, y, df3, iv_df, ivs, Ginis, table, scores, feat, features_of_model, clf_lr, 
                    name='Scoring'):
    #WRITING
    ...
    try: worksheet.write('C18', str([round(sc, 2) for sc in scores]))
